-- =============================================================================
-- CoreForge e-commerce — single-run schema for Supabase (Bolt.new / SQL Editor)
-- Aligns with: src/types/index.ts, useProducts, useOrders, useCreateOrder, authStore
-- Run once in Supabase → SQL Editor → New query → paste → Run
--
-- If CREATE TRIGGER fails on "PROCEDURE", replace EXECUTE PROCEDURE with
-- EXECUTE FUNCTION (PostgreSQL 14+ spelling) for each trigger below.
-- =============================================================================

CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- -----------------------------------------------------------------------------
-- PRODUCTS (matches storefront Product type + admin)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- ORDERS (matches Order in src/types/index.ts)
-- -----------------------------------------------------------------------------
...
CREATE TRIGGER tr_orders_set_number
  ...
  FOR EACH ROW
  EXECUTE PROCEDURE public.set_order_number();
...
CREATE TRIGGER tr_orders_bump_customer
  ...
  FOR EACH ROW
  EXECUTE PROCEDURE public.bump_customer_stats_from_order();

-- -----------------------------------------------------------------------------
-- ORDER ITEMS (matches OrderItem in src/types/index.ts)
-- -----------------------------------------------------------------------------
...
CREATE TRIGGER tr_order_items_decrement_stock
  ...
  FOR EACH ROW
  EXECUTE PROCEDURE public.decrement_product_stock();
...
-- -----------------------------------------------------------------------------
-- AUTH: auto-create customer row (use with app signUp — do not duplicate insert client-side)
-- -----------------------------------------------------------------------------
...
-- Products: public sees active; admins see/manage all
...
-- Orders: customers see their orders; admins see all; guest checkout insert
...
-- -----------------------------------------------------------------------------
-- Make first admin (replace email after seeding)
-- UPDATE public.customers SET role = 'admin' WHERE email = 'you@company.com';
-- -----------------------------------------------------------------------------
